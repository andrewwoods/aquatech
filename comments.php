...
<div class="section" id="respond">
	...
	<?php else : ?>
		...
		<?php else : ?>
			...
			<div>
			<label for="author" <?php if ($req) _e(' class="required" '); ?>><?php _e('Name'); ?> <?php if ($req) _e('(required)'); ?></label>
			<input type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" size="22" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> />
			</div>

			<div>
			<label for="email"><?php _e('E-mail [will not be published]'); ?> <?php if ($req) _e('(required)'); ?></label>
			<input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" <?php if ($req) _e(' class="required" '); ?> tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />
			</div>
		...
		<div><input name="submit" type="submit" id="submit" tabindex="5" value="<?php esc_attr_e('Send Comment'); ?>" />
 ...
 <?php else : // this is displayed if there are no comments so far ?>
	...
	<?php if ( comments_open() ) : ?>
	 ...
	 <?php else : // comments are closed ?>
		<div class="message no-comments">
		<!-- If comments are closed. -->
		<p><?php _e('Sorry, but <strong>comments are closed</strong>. I hope you enjoyed the article'); ?></p>
		</div>
